...
あなたがもし GitHub アカウントを作った時は、真っ先にこの設定を有効にすることをおすすめします。ふつう公開リポジトリに実メールアドレスを埋め込むことでただちに深刻な危険が生じるわけではありませんが、「いいこと」はほとんどありません。

:::warning GitHub と Gitea の設定の両立

Gitea が提供するダミーアドレスは `@trap.jp` で終わる形式ですが、GitHub が提供するダミーアドレスは `@users.noreply.github.com` で終わる形式です。GitHub のダミーアドレスを `git config` でグローバルに設定すると、Gitea 上のリポジトリにコミット・プッシュしたとき Gitea はメールアドレスからユーザーを特定できず、以下のようにアイコンが適切に表示されなくなります。
