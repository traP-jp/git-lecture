...
などの特徴があることが分かりました。ただし、今回 Downloads と Desktop の 2 ヶ所に mydrive を用意したのは異なるデバイスから Git リポジトリにアクセスすることを想定したシミュレーションに過ぎず、通常はデバイスにつき一つだけディレクトリを用意すれば事足りるし面倒もありません。
